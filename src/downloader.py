...
async def downloadSeries(SeriesId: str) -> None:
    url = (
        f"{config.host}/emby/Shows/{SeriesId}/Seasons?api_key={config.api_key}"
    )

    data = (await GET(url)).json()

    selected_seasons = input(
        "请选择要下载的季数(以空格为隔, 用-输入范围, 输入a下载整剧):\n"
    )
...
async def downloadSeasons(
    SeriesId: str, SeasonId: str, SeasonName: int
) -> None:
    ...
    selected_episodes = input(
        "请选择要下载的集数(以空格为隔, 用-输入范围, 输入a下载整季):\n"
    )

    indexs, EpisodeIds = fliterDownloadMediaIds(
        selected_episodes, data["Items"]
    )

    save_dir = Path(f"{SeriesName}/第 {SeasonName} 季")

    for i, EpisodeId in zip(indexs, EpisodeIds):
        media_type, media_size, media_stream = await get_media_stream(
            EpisodeId
        )
...
async def saveMedia(
    ...
    **kwargs,
    ...
    try:
        ...
        memorys["only_me"] = [
            (str(save_dir), str(save_file), media_stream, size)
        ]
...
async def search_media(
    key: str, only_series: str, only_episode: str, only_movies: str
) -> Dict[str, Any]:
    ...
    for media in medias:
        match media.type:
            case "Series":
                ...
                print(
                    f"{[media.index]} 剧名: {media.name}{seasons}季 Id: {media.id}"
                )
            case "Episode":
                print(
                    f"{[media.index]} 集名: {media.name} - {media.series_name}[s{media.season}e{media.episode}] Id: {media.id}"
                )
            case "Movie":
                print(
                    f"{[media.index]} 电影名: {media.name} Id: {media.id} 发布于: {media.year}"
                )
...
async def get_media_stream(media_id: str) -> Tuple[str, int, str]:
    ...
    return (
        type_,
        size_,
        f"{config.host}/videos/{media_id}/stream.{type}?api_key={config.api_key}&MediaSourceId={mediasource_id }&SubtitleStreamIndex={subtitle_index}&Static=true",
    )
...
def fliterDownloadMediaIds(
    inputs: str, items: List[Dict[str, Any]]
    ...
    if inputs.lower() == "a":
        ...
        choiceIds = [
            item["Id"] for item in items if item["IndexNumber"] in indexs
        ]
    else:
